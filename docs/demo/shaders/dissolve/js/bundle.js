...
void main(){
    ...
    var uniforms$1;
    (function (uniforms) {
        uniforms["u_NoiseTex"] = "u_NoiseTex";
        uniforms["u_DissolveThreshold"] = "u_DissolveThreshold";
    })(uniforms$1 || (uniforms$1 = {}));
    class MGShader extends CustomValue2d {
        initialize() {
            this.createShader(vs, ps$1);
        }
    }
    class MGSprite extends CustomSprite {
        constructor() {
            super(MGShader);
        }
        set noiseTexture(v) {
            this.setShaderValue(uniforms$1.u_NoiseTex, v._getSource());
        }
        set dissolveThreshold(v) {
            this.setShaderValue(uniforms$1.u_DissolveThreshold, v);
        }
    }

    class Shader2d_Test2 {
        constructor() {
            this.create();
        }
        create() {
            return __awaiter(this, void 0, void 0, function* () {
                let noise = yield this.loadImage("res/noise.jpg");
                let grass = yield this.loadImage("res/grass.png");
                var mg = new MGSprite();
                mg.texture = grass;
                mg.noiseTexture = noise;
                mg.dissolveThreshold = 1;
                mg.x = Laya.stage.width / 2 - mg.texture.width / 2;
                mg.y = Laya.stage.height / 2 - mg.texture.height / 2;
                Laya.stage.addChild(mg);
                Laya.timer.loop(1, this, function (res) {
                    res.dissolveThreshold = Math.abs(Math.sin(Laya.timer.currTimer / 1000));
                }, [mg]);
            });
        }
        loadImage(url) {
