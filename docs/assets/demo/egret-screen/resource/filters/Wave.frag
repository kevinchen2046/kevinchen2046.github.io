...
void main(){
    ...
    gl_FragColor=texture2D(uSampler,vec2(vTextureCoord.x+amplitudeValue*.05,vTextureCoord.y))+
                    (texture2D(uSamplerNoise,vec2(vTextureCoord.x+amplitudeValue,vTextureCoord.y+speed))+vec4(0.3,0.2,0.0,0.0))*.3;
